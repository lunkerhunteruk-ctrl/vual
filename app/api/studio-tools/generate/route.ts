...
const TEMPLATES: Record<string, { cat: string; desc: string; template: string }> = {
  ...
  PD5: { cat: 'Provocative D', desc: 'Collar grip — hand pulling own collar/neckline', template: `PROVOCATIVE SHOT — COLLAR GRIP:\nCOMPOSITION: Upper body close-up. The model's hand GRIPS her own collar, neckline, or lapel — pulling it slightly, creating tension in the fabric. The knuckles are visible, the pull is deliberate. This is not adjusting clothing — it is a gesture of restrained violence toward her own garment, as if she might tear it.\nEXPRESSION: Looking directly at camera. The eyes are calm but the hand tells a different story. The contradiction between the controlled face and the aggressive hand is the tension.\nLIGHTING: Close-range directional light emphasizing the stretched fabric and the tendons in the gripping hand.\nLENS: Shot on Canon RF 85mm f/1.2L USM DS — the fabric tension and skin texture rendered with luminous detail.` },
...
export async function POST(req: NextRequest) {
  const { story, height, tuck, outer, shotCount = 6, hasBag = false, hasShoes = false, filmMode = false, provocative = false, sculptural = false, surveillance = false, cyberNeon = false, neonColor = 'mix', neonIntensity = 'normal', timeOfDay = 'day', tights = 'none' } = await req.json();
...
IMPORTANT: This is a fashion photograph with the gravitas of fine art — like Peter Lindbergh shooting color. The medium format rendering elevates every texture, every pore, every fabric fold into something monumental. The image should feel heavy, significant, timeless.`,
  ...
  const isDailyLife = !surveillance && !sculptural && !provocative &&
    dailyLifeKeywords.some(kw => storyLower.includes(kw.toLowerCase()));
  ...
  if (!hasShoes) excludeIds.push('DC2', 'DC5', 'DC6'); // shoe-focused shots
...
${isCyberDaily ? `■ CYBER DAILY MODE — DAILY LIFE ACTIONS IN NEON:
...
- MANDATORY MIX: You MUST include at least 2 templates from Provocative E (PE1-PE7) in every series. These "inhuman indifference" shots are what separates this editorial from conventional fashion photography.
...
[
  ...
  ...
    ...
    : isDailyLife
    ? `You are a fashion editorial director creating a ${shotCount}-shot "Tokyo Daily" series — high-fashion editorial photography set in REAL, MUNDANE, EVERYDAY locations. The concept: MODE × NICHIJŌ (モード×日常) — place haute couture in the most ordinary moments of daily life.
...
The model is ENTITY_01 (Rin) — an AI entity inhabiting a physical form. She wears extraordinary high-fashion garments in utterly ordinary places. The friction between MODE and NICHIJŌ is the concept.
...
- "She stands at a pachinko machine, balls cascading around her. Every other player is hunched and frantic. She sits upright, hands in her lap, staring straight ahead."

WEAK shots (generic, no tension):
- "She stands in the convenience store." (no action, no tension)
- "She walks through the space looking around." (too vague)
- "She browses the shelves." (boring, no visual hook)
...
ACCESSORIES RULE: The model must NOT wear any rings, bracelets, necklaces, or jewelry unless they are explicitly provided as reference images.${
  ...
  !hasShoes ? '\nSHOES NOTE: If shoes are provided in the reference images, reproduce them exactly. The model must ALWAYS wear shoes — never barefoot.' : ''
}
...
GARMENT SHAPE RULE: Do NOT add, extend, or modify the garment silhouette. If the reference shows a jacket, do NOT add flowing fabric, trailing hem, or cape-like extensions. If the reference shows a short skirt, do NOT make it long. The garment's LENGTH, SHAPE, and SILHOUETTE must match the reference images EXACTLY. No phantom fabric.`;
  ...
  const shots = plan.map((p) => {
    ...
    return {
      ...
      prompt: `${basePrompt}\n\n${template}\n\nSCENE-SPECIFIC NOTE: ${p.customNote}`,
    };
  }).filter(Boolean);
