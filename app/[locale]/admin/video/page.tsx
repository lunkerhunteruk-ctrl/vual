    ...
    try {
      ...
      const res = await fetch('/api/video/render', {
        ...
        body: JSON.stringify({
          ...
          introStyle: settings.introStyle,
          introText: settings.introText,
          flatLayImageUrls: settings.flatLayImages.some(u => u) ? settings.flatLayImages.filter(u => u) : undefined,
          locationText: settings.locationText,
          dateText: settings.dateText,
          showEnding: settings.showEnding,
          whiteFlash: settings.whiteFlash,
          brandName: settings.showCredit ? (store?.name || 'VUAL') : undefined,
          tagline: settings.endingText || undefined,
      ...
      <div className="flex-1 min-w-0">
        {selectedBundle ? (
          <div className="space-y-4">
            ...
            {/* Dev mode: Import Clips */}
            {isDevStore && (
              <div className="border-t border-[var(--color-line)] pt-5 mt-6">
                ...
                {/* Duration summary + Render button */}
                <div className="flex items-center gap-3">
                  {importedClipCount > 0 && (
                    <div className="text-[10px] text-[var(--color-text-label)] font-mono">
                  ...
                  <div className="ml-auto">
                    ...
                    >
                      {isImportRendering
                        ...
                        : importFinalVideoUrl
                          ? (ja ? '再レンダリング' : 'Re-render')
                          : (ja ? 'インポートをレンダリング' : 'Render Imports')}
                    </Button>
                  </div>
                </div>
