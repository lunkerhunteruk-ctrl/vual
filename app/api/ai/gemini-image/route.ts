...
interface RequestBody {
  ...
  offshotVariant?: 'A' | 'B' | 'C'; // A = on-set BTS, B = after-party/nightlife, C = morning/daytime
  shotIndex?: number;
  totalShots?: number;
...
- The overall feeling is fresh, optimistic, real — like a photo from a model's "day in my life" Instagram story.`;
    ...
    const offshotCCategories = [
      // Shot 1: HOTEL BREAKFAST BUFFET
      {
        ...
        location: `A hotel breakfast buffet restaurant in ${cityName || 'the shooting location'}. ${cityContextC} A comfortable 3-star or 4-star hotel — clean, welcoming, approachable (NOT a luxury 5-star hotel). Morning light through large windows, other hotel guests visible in background. Buffet counter with food trays, fresh bread, fruits, cereals.`,
      ...
      // Shot 2: HOTEL LOBBY — CANDID STAFF SHOT
      {
        ...
        wardrobe: `${garmentDesc}${secondGarmentDesc}${thirdGarmentDesc}${fourthGarmentDesc}${fifthGarmentDesc} — dressed and ready for the day. Bag over shoulder or in hand`,
        location: `The lobby of a 3-star or 4-star hotel in ${cityName || 'the shooting location'}. ${cityContextC} NOT a luxury grand hotel — a clean, comfortable, approachable hotel with a welcoming atmosphere. Simple but tidy lobby with reception desk, seating area, maybe a small coffee station. The hotel should feel like a real working crew would actually stay here. Bright morning light from entrance.`,
      ...
      // Shot 4: HOTEL CHECKOUT / LOBBY
      {
        ...
        wardrobe: `${garmentDesc}${secondGarmentDesc}${thirdGarmentDesc}${fourthGarmentDesc}${fifthGarmentDesc} — IMPORTANT: The garments, shoes, and all accessories must be EXACTLY as shown in the reference images. Sunglasses pushed up on her head or hanging from the collar. A tote bag or small carry-on`,
        location: `The lobby or front desk of a 3-star or 4-star hotel in ${cityName || 'the shooting location'}. ${cityContextC} Clean and comfortable hotel interior — NOT a luxury grand hotel. Morning activity — other guests checking out, staff behind the desk. Bright natural light from the entrance.`,
        film: leicaMorning, quality: qualityBright,
      ...
      // Shot 5: TAXI / CAR RIDE TO LOCATION
      {
        ...
        wardrobe: `${garmentDesc}${secondGarmentDesc}${thirdGarmentDesc}${fourthGarmentDesc}${fifthGarmentDesc} — IMPORTANT: The garments, shoes, and all accessories must be EXACTLY as shown in the reference images. Sunglasses on or pushed up, bag on the seat beside her`,
        location: `Inside a taxi or car driving through the busy downtown area near ${cityName || 'the shooting location'}. ${cityContextC} Local street signs, shops, architecture, and morning commuters visible through windows. The route passes through a recognizable commercial or entertainment district.`,
      ...
      // Shot 6: CAFE — COFFEE & LUNCH
      {
        ...
        wardrobe: `${garmentDesc}${secondGarmentDesc}${thirdGarmentDesc}${fourthGarmentDesc}${fifthGarmentDesc} — IMPORTANT: The garments, shoes, and all accessories must be EXACTLY as shown in the reference images. Sunglasses on top of head or on the table. A light jacket or bag draped over the chair`,
        location: `A local cafe near ${cityName || 'the shooting location'}. ${cityContextC} Authentic local cafe — not a chain. Local pastries or food in the display, local language on the menu board, neighborhood regulars in the background. Bright daylight, outdoor terrace or large windows. Plants, wooden furniture, warm atmosphere.`,
        film: leicaMorning, quality: qualityBright,
        expression: 'Looking directly at camera. Relaxed, happy, genuine — enjoying a real moment before work begins.',
      },
    ];

    const shotIdxC = typeof body.shotIndex === 'number' ? body.shotIndex % offshotCCategories.length : 0;
    const categoryC = offshotCCategories[shotIdxC];
    const actionC = pick(categoryC.actions);

    return `${offshotCDirective}

${categoryC.film}
${modelDescription}
The model is ${categoryC.wardrobe}.
${categoryC.location}

SCENE: The model ${actionC}.
EXPRESSION: ${categoryC.expression}
${categoryC.quality}
