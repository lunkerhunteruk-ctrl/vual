...
export default function TryOnPage() {
  ...
  const { customer, isCustomerLoading, setCustomer } = useAuthStore();
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (googleCustomer) setCustomer(googleCustomer, 'google');
    } catch {
      // ignore
    } finally {
      ...
      <div className="min-h-[70vh] flex flex-col items-center justify-center px-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center max-w-sm w-full">
          ...
          <p className="text-sm text-[var(--color-text-body)] mb-8">
            {isJa ? 'ログインしてバーチャル試着をお楽しみください' : 'Log in to enjoy virtual try-on'}
          </p>
          <button
            onClick={liffLogin}
          ...
          >
            {isJa ? 'LINEでログイン' : 'Log in with LINE'}
          </button>
          <button
          ...
          >
            ...
            {isJa ? 'Googleでログイン' : 'Log in with Google'}
          </button>
        </motion.div>
      </div>
    );
