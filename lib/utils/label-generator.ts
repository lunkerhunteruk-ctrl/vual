...
export async function generateLabel(params: LabelParams): Promise<Buffer> {
  ...
  const variantSvg = variantLine
    ? `<text x="${TEXT_X}" y="${variantY}" font-family="'Helvetica Neue', Arial, sans-serif" font-size="18" fill="#555555">${variantDisplay}</text>`
    : '';
  ...
  <text x="${TEXT_X}" y="${nameY}" font-family="'Helvetica Neue', Arial, sans-serif" font-size="22" font-weight="600" fill="#1A1A1A">${nameDisplay}</text>
  ${variantSvg}
  <text x="${TEXT_X}" y="${priceY}" font-family="'Helvetica Neue', Arial, sans-serif" font-size="24" font-weight="700" fill="#1A1A1A">${priceDisplay}</text>
  <text x="${TEXT_X}" y="${skuY}" font-family="'Helvetica Neue', Arial, sans-serif" font-size="13" fill="#999999" letter-spacing="0.5">SKU: ${skuDisplay}</text>
  <text x="${LABEL_W - 16}" y="${LABEL_H - 16}" text-anchor="end" font-family="'Helvetica Neue', Arial, sans-serif" font-size="11" fill="#BBBBBB" letter-spacing="0.3">${storeDisplay}</text>
</svg>`;
